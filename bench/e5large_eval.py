#!/usr/bin/env python3
"""Offline eval: does intfloat/multilingual-e5-large-instruct fix swctx's dead
semantic leg on Vietnamese queries — and does it escape bge-m3's latency trap?

Pure measurement — embeds index chunks + vn_queries with
multilingual-e5-large-instruct (XLM-RoBERTa-large, 24L, 1024-dim; model-card
convention: queries get "Instruct: {task}\\nQuery: {query}", documents get NO
prefix; mean pooling over non-pad tokens + L2 norm) and reports cosine rank of
expected-file chunks.

Embeddings cached under /tmp/e5large_eval/*.npy so re-runs are cheap.

Usage:
  e5large_eval.py                  # full eval on MPS (queries, corpus, ranks;
                                   #   also benches single-query MPS latency)
  e5large_eval.py --latency-only --device cpu
                                   # only bench single-query latency on the
                                   #   given device -> /tmp/e5large_eval/latency_<dev>.json
"""
import argparse
import json
import os
...
# e5-instruct model-card convention — query-side instruction is MANDATORY
# (card FAQ: "Yes, this is how the model is trained"); documents get NO prefix.
INSTRUCT_TASK = (
    "Given a code search query, retrieve relevant code and documentation chunks"
)


def q_wrap(query: str) -> str:
    return f"Instruct: {INSTRUCT_TASK}\nQuery: {query}"


P_PREFIX = ""  # no document-side prefix for instruct models (per model card)
...
def embed_texts(texts, tok, model, device, label, checkpoint=None, quiet=False):
    ...
    Returns float32 [N, hidden]. Callers must pre-apply the instruct format."""
    n = len(texts)
...
def main():
    ...
    if os.path.exists(q_cache):
        ...
        t0 = time.time()
        q_emb = embed_texts(
            [q_wrap(q["query"]) for q in queries], tok, model, device, "queries"
        )
