#!/usr/bin/env python3
"""rerank_eval_v3.py — jina-reranker-v2-base-multilingual spike evaluation for swctx.

Identical methodology to bench/rerank_eval_v2m3.py but invokes
`swctx rerank3` (jina XLM-R-base cross-encoder) instead of
`swctx rerank2` (bge-reranker-v2-m3) / `swctx rerank` (amberoad
mBERT). Baseline is still `swctx search --mode auto`.

    python3 bench/rerank_eval_v3.py                  # table on stdout
    python3 bench/rerank_eval_v3.py --json           # machine-readable
    python3 bench/rerank_eval_v3.py --out file.json  # also save raw data
"""
...
def main():
    ...
    ap.add_argument("--rerank-cmd", default="rerank3",
                    help="swctx subcommand used for the rerank leg")
    ...
    for i, r in enumerate(queries):
        ...
        rr, rr_ms, rr_err = run_cli(
            [args.swctx_bin, args.rerank_cmd, ws, q,
             "--limit", str(args.pool)])
