...
/// Cross-encoder rerank stage (spike v2): BAAI/bge-reranker-v2-m3 —
/// XLMRobertaForSequenceClassification on the bge-m3 backbone
/// (24L/1024H, ~568M params, ONE relevance logit; score = logit[0]).
///
/// Weights: ~/.swctx/models/bge-reranker-v2-m3/
///   {model.mlpackage, sentencepiece.bpe.model} — converted by
///   bench/convert_reranker_v2m3.py. The mlpackage declares two inputs
///   (input_ids, attention_mask) at (1, RangeDim(1,1024)) — XLM-R has
///   no token_type_ids.
///
/// Pair encoding is the XLM-R convention `<s> q </s></s> d </s>`:
///   [0] + q + [2,2] + d + [2]
/// built from SPTokenizer (unigram) ids with the HF fairseq mapping:
///   spm unk(0) → 3, every other spm id → id + 1
/// (spm piece table is shifted one slot vs HF's — <s>/<pad>/</s>/<unk>
/// occupy HF ids 0-3 while spm keeps <unk>/<s>/</s> at 0-2).
///
/// Batching mirrors Reranker: MLModel.predictions(fromBatch:) over the
/// batch-1 model (dynamic-batch conversion produced input-invariant
/// logits on the amberoad spike — same workaround).
public final class RerankerV2: @unchecked Sendable {
    public static let dirName = "bge-reranker-v2-m3"
    ...
    /// 568M params, so callers share one instance. `nil` when the model
    /// isn't installed or fails to load.
    public static var shared: RerankerV2? {
        lock.lock(); defer { lock.unlock() }
        if let cached { return cached }
        guard isInstalled else { return nil }
        cached = try? RerankerV2()
        return cached
    }
    private static let lock = NSLock()
    nonisolated(unsafe) private static var cached: RerankerV2?
    ...
    /// mlpackage seq axis is RangeDim(1,1024) but CPU inference cost
    /// grows ~quadratically (~5s @ 512 → ~12s @ 1024), so we cap at
    /// FlagReranker's own default of 512.
