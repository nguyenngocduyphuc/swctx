...
/// Tests for `RerankerV2` (bge-reranker-v2-m3, XLM-R cross-encoder).
///
/// `tokenizePair` is asserted byte-for-byte against HF
/// `XLMRobertaTokenizer` pair encodings captured from transformers
/// 4.46.3 on the real model (see bench/convert_reranker_v2m3.py):
...
final class RerankerV2Tests: SwctxTestCase {
    ...
    private func loadReranker(
        ...
        guard RerankerV2.isInstalled else {
            try XCTSkipUnless(false,
                "bge-reranker-v2-m3 not installed at \(RerankerV2.modelDir.path)",
                file: file, line: line)
            return nil
        }
        return try RerankerV2()
    }
    ...
    func testTokenizePairTruncation() throws {
        ...
        XCTAssertEqual(ids.count, RerankerV2.maxTokens)
